...
- **OAuth refresh token + client_credentials grant (v1.13 adayı)** — `exchangeRefreshToken` + RFC 9700 rotation (migration v4 `refresh_token_hash`, revoke-on-reuse); M2M client_credentials.
- ✅ ~~consume-side roots + abort-to-host~~ — **Faz 20 v1.11'de YAPILDI**.
- ✅ ~~roots/list upstream agregasyonu + abort propagation (ReAct loop + providers)~~ — **Faz 21 v1.12'de YAPILDI** (`getFederatedRoots` + expose `roots/list`; `buildSignal` + client-disconnect abort). Kalan: expose-side sampling'i somut bir tool'a bağlama + **resource subscriptions** (stateless transport sınırı).
