...
- **MCP sampling + elicitation (v1.9 adayı)** — server→client `createMessage`/`elicit`; YALNIZ bidirectional stdio transport'ta fizibıl (stateless HTTP hariç). SDK `CreateMessageRequest`/`ElicitRequest`.
- **OAuth 2.1 Authorization Server (v1.9 adayı)** — `/oauth/authorize` + `/oauth/token` + PKCE (`node-oauth2-server` MIT), migration v3 token tablosu; DCR (Faz 15B) üstüne tam AS. *Gerçek abort'u host-bridge AbortSignal'ine thread etme (şu an MCP yanıtı prompt iptal, alt çağrı timeout'a kadar sürer).*
