...
- **OAuth 2.1 Authorization Server (v1.10 adayı)** — `/oauth/authorize` + `/oauth/token` + PKCE (`@node-oauth/oauth2-server` MIT, migration v3 `oauth_codes`/`oauth_tokens`, jose imzalama mevcut); DCR (Faz 15B) üstüne tam AS. HTTP-only.
- **roots/list upstream agregasyonu** + expose-side sampling'i somut bir tool'a bağlama + gerçek abort'u host-bridge AbortSignal'ine thread (şu an MCP yanıtı prompt iptal, alt çağrı timeout'a kadar sürer).
