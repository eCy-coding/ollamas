...
- **OAuth refresh token + client_credentials grant (v1.12 adayı)** — `exchangeRefreshToken` + RFC 9700 rotation; M2M client_credentials.
- ✅ ~~consume-side roots + gerçek abort'u host-bridge AbortSignal'ine thread~~ — **Faz 20 v1.11'de YAPILDI** (`AbortSignal.any` + `callTool` signal). Kalan: **roots/list upstream agregasyonu** (client'ın root'larını okuyup FS tool'larını scope'lama) + expose-side sampling'i somut bir tool'a bağlama + **resource subscriptions** (stateless transport sınırı).
