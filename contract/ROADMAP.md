# CONTRACT LANE ROADMAP

| Ver | Tema | Durum |
|---|---|---|
| **vK1** | Kontrat çekirdeği — doc hash, registry state machine, ed25519 identity, atomic state | ✅ DONE |
| **vK2** | Key köprüsü + server wiring (/api/contract/*) + contract_admin tool + CLI | planned |
| **vK3** | Pool ledger + heartbeat + canlı doctor → kontrat bölümü TAMAM | planned |
| **vK4** | Scheduler federasyon — pool skoru → selectFleetBackend, gateway kota | planned |
| **vK5** | Partition hesabı — exo memory-weighted layer dilimi (PURE) | planned |
| **vK6** | RPC sharding — llama.cpp rpc-server grup, shard-group sanal-backend | planned |

## vK2 — NEXT
