...
> Auto: `tsx orchestration/bin/completion-scan.ts` · 2026-07-02T14:43:13Z. The council's end-to-end scan of ollamas:
...
- **98 .mjs files still to migrate → TypeScript** (owner: `mjs-migration`; in-place `@ts-check` progress: **4/98**).
  - scripts: 31
...
### `mjs-migration` (1)
- [P1] 98 .mjs files still to migrate to TypeScript (4 already `@ts-check`'d) — TS is the primary language (type-safety, single toolchain); un-migrated .mjs escapes tsc + the shared type contracts.
