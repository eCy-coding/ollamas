...
export function buildRoleAnswer(i: RoleInputs): string {
  ...
  return [
    ...
    `- ♾️ **E2E-loop (vO30/31):** autopilot zincirini convergence'a kadar döngüler + convergence-tespit (applied-state reconcile → CONVERGED erişilebilir) — \`tsx bin/loop.ts\` (\`/loop\`) → \`docs/E2E_LOOP.md\``,
    `- 🧭 **Sequenced-mission (vO32):** paralel fleet → adım-adım (T1→Tn) bağımlılık-sıralı + etik-tier (asla privileged, ≤2/model) — \`tsx bin/mission.ts\` (\`/mission\`) → \`orchestration/MISSION.md\``,
    ``,
