...
import { buildRow, renderChromeProbe, providerFor, type ChromeProbeRow, type DispatchReport } from "./lib/chrome-probe";
...
function main(): void {
  ...
  if (DRY) {
    console.log(`chrome-probe (dry) — ${models.length} model, sıralı. Server: ${OLLAMAS_URL}`);
