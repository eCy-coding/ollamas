...
## Faz 32 — v1.20 MCP Resource Subscriptions (branch feat/v1.20-resource-subscriptions, off integration/all-lanes, zero-dep)
- **Ne:** SEYIR'in önceden-hesapladığı next (Faz 28 satır 224). MCP `resources/subscribe` + `resources/unsubscribe` + `notifications/resources/updated`. Gateway artık workspace dosyası değişince canlı push yapar (önceden sadece resources/list+read). **0 yeni dep** (SDK 1.29 zaten `SubscribeRequestSchema`/`UnsubscribeRequestSchema`/`sendResourceUpdated` veriyor + node built-in `fs.watch`). VIBE YOK — spec-driven, proven kaynak: resmi MCP spec + @modelcontextprotocol/sdk + modelcontextprotocol/servers reference.
- **Nasıl:** `server/mcp/subscriptions.ts` (NEW) `SubscriptionRegistry`: `fs.watch` + 150ms debounce (editor multi-write coalesce), `FilesystemManager.resolveSafePath` ile traversal-guard'lı abs-path çözümü, subscribe/unsubscribe/dispose/size. `server/mcp/server.ts`: 2 handler + capability `resources:{subscribe:true}` + per-buildServer registry, callback `server.sendResourceUpdated({uri})`, `server.onclose → registry.dispose()` (her iki transport'ta watcher-leak yok).
- **Transport (SEYIR planına birebir):** stdio persistent → subscriptions tam çalışır (canlı push). HTTP stateless per-request → subscribe kabul edilir ama kanal response ile kapanır (best-effort, belgeli) + onclose dispose.
- **Güvenlik invariant:** subscribe URI workspace-içi çözülmeli (resolveSafePath); traversal (`file://../etc/passwd`) ve eksik-dosya → temiz JSON-RPC error. Unit + e2e ile kanıtlı.
- **Kanıt:** lint clean · build ok · **full suite 116 dosya/803 test PASS** (+6: 5 unit registry + 1 stdio-e2e) · conformance:stdio 3/3 · **CANLI real-time e2e**: gerçek `bin/mcp-stdio.ts` (tsx) spawn → SDK client `subscribeResource` → diskte dosya değiştir → gerçek `notifications/resources/updated{uri}` alındı → `unsubscribeResource` → sustu. 
- **Prevention (RISK-MCP-TEST):** MCP client testlerinde `setNotificationHandler` Zod-şeması ister (plain `{method}` obj kaydolmaz) + `request()` 2. arg result-şeması olmalı → SDK helper'ları kullan (`subscribeResource`/`ResourceUpdatedNotificationSchema`), raw request değil.
- **Sonraki (önceden hesaplandı):** rate-limit/quota enforcement VEYA OpenTelemetry tracing (araştırılan roadmap). Ayrı: dist/server.cjs `import.meta.url` build-integrity fix (prod tsx kullandığı için acil değil); ertelenen lane'ler gateway-v2/v1.8-bench/general-oauth operatör review.
