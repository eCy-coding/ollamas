...
| **vC1.6** | HİBRİT triage — local ilk-geçiş (qwen3-coder, 0-manuel, blok yok) + Gemini verify-geçişi `isReal` bulgularda (runtime+izin varsa, yoksa local'e düşer) + per-finding `triage.log` tracking. Egress=onaylı, escalated-only | ✅ DONE (local 0-manuel canlı kanıtlı; Gemini-verify izin-sonrası oto) |
