...
| Confidence/provenance | **ON** | — | `confidence` kolonu: teach 0.95 / system 0.9 / user 0.85 / mirror 0.7 / retain 0.6 / **LLM-distill 0.5 (untrusted)**; recall skoru ×(0.55+0.45c); ask düşük-güvene ihtiyat kuralı |
| Fact write-behind | **ON** | `BRAIN_DEFER_EMBED=0` | assertFact embed-bütçesini aşarsa fact SATIRI yine yazılır (`brain_facts.pending_embed=1`, süperseed SQL-only zaten çalışır); vektör backfill'de gelir — fact kaybı sınıfı kapandı |
