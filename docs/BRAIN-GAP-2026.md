...
| 3 | Temporal reasoning / event ordering | Yes | ◐→✓ | facts bi-temporal + point-in-time ✓; **relative-time sorgu çözümleme (B2)** bu turda |
| 4 | Knowledge update / staleness | Yes | ✓ | fact süperseed + belief-revision (`superseded_at`, 50fcd21) |
| 5 | Abstention / grounding threshold | Yes | ✗→✓ | recall eşiksiz k döndürüyordu — **B1 bu turda** |
...
| 8 | Actor attribution (multi-party) | Yes | ✗→✓ | satırda konuşan-kimliği yoktu — **B5 bu turda** |
...
| 13 | Right-to-be-forgotten (delete-by-subject) | Mixed | ✗→✓ | silme API'si yoktu — **B4 bu turda** |
| 14 | Audit trail (append-only ledger) | Yes | ✗→✓ | OTel log vardı ama DB-ledger yoktu — **B3 bu turda** |
