...
- ~~Cross-encoder rerank ertelenmiş~~ → **DEĞERLENDİRİLDİ + ADOPT EDİLDİ (2026-07-18, brain tamamlama turu):** main'in B5 `server/rerank.ts`'i (Xenova/bge-reranker-base, transformers.js ONNX, $0 LOKAL — erteleme gerekçesi "model/$ gerektirir" düştü) `recall()`'a opt-in bağlandı (`BRAIN_RERANK=1`): fused+scored havuzun ilk `max(3k,12)` adayı cross-encoder'la yeniden sıralanır, sonra k'ya kesilir; graceful-degrade rerankCandidates'ta. **Kanıt:** `make eval-rerank` fixture MRR@5 0.2733→0.9333 (+0.66); brain altın-seti `eval-brain-mrr` **0.8562 → 0.9531** (+9.7 puan, 12/16→15/16 top-1). Default OFF kalır (ilk çağrı ONNX indirir/yükler — gate ve soğuk auto-recall yolu için uygunsuz); üretimde `.env BRAIN_RERANK=1` + restart ile açılır.
