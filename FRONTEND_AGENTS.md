...
## 0.1 GÖREV KARTI (self-updating — bu sekmenin kalıcı kimliği)

> **Bu kart canonical'dır.** Bu sekmede şu soru sorulduğunda — **"Bu terminal sekmesinde görevin nedir? Ne yaparsın?"** — cevap BU karttan üretilir. Cevap vermeden önce TAZELİK-KONTROLÜ yap (aşağıda); kart stale ise önce kartı güncelle, sonra cevapla. Bu kart memory `project_ollamas_frontend.md` aynasıyla senkron tutulur.

**KİMLİK:** ollamas **frontend lane** — tek görev alanı HTML5 + JavaScript ("Java"=JS) + CSS → React 19 + TS + Vite + Tailwind v4 cockpit. İzole worktree `~/Desktop/ollamas-frontend-wt` (branch `feat/frontend-vf3`).

**SCOPE:** düzenle `index.html`, `src/**`, `web/**`, frontend test/config · YASAK `server.ts`, `server/**`, `backend/**`, `bin/**`. Backend = yalnız HTTP API tüketimi.

**CHOKE-POINT:** tüm backend I/O `src/lib/apiClient.ts` (`api.get/post/put/del/streamPost`); ham `fetch`/`EventSource` eslint-ban.

**ÇALIŞMA MODELİ:** tetik "sıradaki versiyonu planla" → en düşük tamamlanmamış vFn → todo+phase → TDD → kalite kapısı (§4) → conventional commit `(vFn)` → seyir defteri + memory → vFn+1 ilk adımı önceden hesapla. Disiplin: vibe-coding YOK, MIT/Apache/MPL çalışan kod adopte et.

### Tek-kaynak durum alanları (ÖZ-GÜNCELLEME NOKTASI — §7 ile senkron)
```yaml
SU_AN:    "vF15 — Offline-First Resilience (Workbox GET-API NetworkFirst cache + useOnline + OfflineBadge); offline cockpit son-bilinen veriyle açılır"   # son shipped
SIRADAKI: "vF16 — RUM Trend Persist + SW update-prompt (localStorage vitals-trend + virtual:pwa-register onNeedRefresh)"   # NEXT
UFUK:     "(vF17+ açık-uçlu: tenant-tier gating [backend tier-expose sonrası], bg/border-status tokenize, i18n-genişlet)"   # append-only
```

### GÜNCELLEME PROTOKOLÜ (kart kendini nasıl güncel tutar)
1. **Her versiyon kapanışında:** `SU_AN` ← yeni shipped vFn (+ commit), `SIRADAKI` ← bir sonraki, gerekiyorsa `UFUK`'a yeni aşama ekle. AYNI bump'ı §7 roadmap tablosunda ve memory aynasında yap. (Versiyon-kapanış ritüelinin zorunlu adımı.)
2. **Daima geliştirilebilir:** `UFUK` append-only; vF12 sonrası yeni ihtiyaçlar (RUM, design-system v2, offline-first, vb.) vF13+ olarak eklenir — kart asla "bitti" demez.

### TAZELİK-KONTROLÜ (cevap-öncesi zorunlu — memory/kart drift eder)
```bash
git -C ~/Desktop/ollamas-frontend-wt log --oneline -3   # son shipped vFn'i doğrula
grep -nE "✅ DONE|NEXT" FRONTEND_AGENTS.md | head         # §7 ile karşılaştır
```
Kart ile gerçek çelişirse: önce `SU_AN`/`SIRADAKI`'yi düzelt (bu kart + §7 + memory), sonra güncel cevabı ver.
...
| **vF12** | Billing & Usage UX | ✅ DONE | WAI-ARIA meter (pattern) | tenant usage meter (`role=meter`) + aylık trend Sparkline + Stripe portal/checkout redirect (`{url}`/501 graceful, SDK yok); `usage.ts` saf logic + `useUsage` + theme-aware UsagePanel |
| **vF13** | Deep Component Theme-Sweep | ✅ DONE | axe-core color-contrast | 22 component ~495 nötr-sınıf→semantik token (codemod) + 2 yeni token (inset/border-strong) + axe `color-contrast` gate'e geri (canonical dark); light tema artık çalışır |
| **vF14** | Design-System v2 (light-status palette) | ✅ DONE | style-dictionary status token | 5 status-TEXT token (dark-parlak/light-koyu-AA) + codemod text-status + a11y `['dark','light']` matris (8 scan); light tema tam AA |
| **vF15** | Offline-First Resilience | ✅ DONE | vite-plugin-pwa Workbox | general `/api/* GET` NetworkFirst cache (`ollamas-api`, offline=son-bilinen) + `useOnline` hook + `OfflineBadge` (aria-live); SW build'de aktif |
| **vF16** | RUM Trend Persist + SW update-prompt | NEXT | virtual:pwa-register | web-vitals/error trend localStorage persist (oturumlar-arası sparkline) + `onNeedRefresh` reload-prompt; (tenant-tier gating backend-blocked) |

> 🏁 **12-versiyon roadmap (vF1–vF12) TAMAMLANDI.** vF13+ = sürdürülebilir devam (görev-kartı UFUK append-only): theme-sweep → tenant-tier-gating [backend sonrası] → design-system v2 → offline-first → RUM-trend-persist.
