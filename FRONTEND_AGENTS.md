...
```yaml
SU_AN:    "vF13 — Deep Component Theme-Sweep (22 component nötr-renk→semantik token + axe color-contrast gate canonical-dark; light tema çalışır)"   # son shipped
SIRADAKI: "vF14 — Design-System v2 (status-renk light-AA paleti + a11y dark+light matris)"   # NEXT
UFUK:     "(vF15+ açık-uçlu: tenant-tier gating [backend tier-expose sonrası], offline-first, RUM-trend-persist)"   # append-only
```
...
| **vF14** | Design-System v2 (light-status palette) | NEXT | — | status renklerine `[data-theme=light]` AA-varyant (emerald/amber/rose/cyan/indigo) + a11y dark+light matris tara + (backend sonrası) tenant-tier gating |
