...
```yaml
SU_AN:    "vF14 — Design-System v2 (5 status-TEXT token light-AA + a11y dark+light matris 8-scan); light tema TAM erişilebilir"   # son shipped
SIRADAKI: "vF15 — Offline-First Resilience (GET-API SWR cache + navigator.onLine rozeti, vite-plugin-pwa Workbox)"   # NEXT
UFUK:     "(vF16+ açık-uçlu: tenant-tier gating [backend tier-expose sonrası], RUM-trend-persist, bg/border-status tokenize)"   # append-only
```
...
| **vF15** | Offline-First Resilience | NEXT | vite-plugin-pwa Workbox | GET-API StaleWhileRevalidate cache (offline cockpit) + `navigator.onLine` rozeti; (tenant-tier gating backend-blocked, ayrı) |
