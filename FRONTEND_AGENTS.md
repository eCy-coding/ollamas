...
```yaml
SU_AN:    "vF9 — i18n (TR/EN runtime) + Theming (light/dark token-flip, no-flash)"   # son shipped
SIRADAKI: "vF10 — Observability & Self-Heal (client-error→logbook + in-cockpit RUM + regresyon gate)"   # NEXT
UFUK:     "vF11 Tenant-aware Cockpit → vF12 Billing & Usage UX → (vF13+ açık-uçlu: derin theme-sweep, design-system v2, offline-first)"   # append-only
```
...
| **vF10** | Observability & Self-Heal | NEXT | — | client error boundary→/api/logbook + in-cockpit RUM (web-vitals, pure-SVG) + perf/görsel regresyon gate |
