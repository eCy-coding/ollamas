...
```yaml
SU_AN:    "vF12 — Billing & Usage UX (tenant usage meter + Stripe portal/checkout redirect) · 🏁 12-versiyon roadmap TAMAMLANDI"   # son shipped
SIRADAKI: "vF13 — Deep Component Theme-Sweep (13 component hardcoded renk→token + axe color-contrast gate'e geri)"   # NEXT
UFUK:     "(vF14+ açık-uçlu: tenant-tier gating [backend tier-expose sonrası], design-system v2, offline-first, RUM-trend-persist)"   # append-only
```
...
| **vF13** | Deep Component Theme-Sweep | NEXT | axe-core color-contrast | 13 component hardcoded renk→token-utility migrate + axe `color-contrast` kuralını gate'e geri al (vF9 theming'i dürüstçe tamamlar) |
