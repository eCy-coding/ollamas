...
| D11 | UX/performans | Lighthouse (`npx lhci autorun` — lighthouserc.json `./dist` üretim-build'ini hedefler; literal `:3000` dev-server tsx/minify-siz olduğundan yanıltıcı 0.25 verir, ölçüm-aracı uyumsuzluğu S-017) | `lighthouserc.json` eşikleri KARŞILANIR — eşik MEVCUT (perf≥0.85, LCP≤2500, CLS≤0.1, TBT≤300) + `budget.json` (script 150KB/total 200KB); yalnız RUN+doğrula (M-018 ⊘-config) | P4 | ✅ lhci exit-0 (M-044 canlı) + perf 0.96 (M-018) |
