...
export const openApiSpec = swaggerJsdoc({
  definition: {
    ...
    paths: {
      ...
      "/api/brain/remember": { post: { tags: ["Agent"], summary: "Brain write choke-point for out-of-process producers (org conductor mirror, imports): explicit ids upsert idempotently, auto-ids pass AUDN dedup, createdAt preserves event time", requestBody: { required: true, content: { "application/json": { schema: { type: "object", required: ["tier", "content"], properties: { id: { type: "string" }, tier: { type: "string", enum: ["core", "learned", "procedural", "episodic", "working"] }, content: { type: "string" }, source: { type: "string" }, ns: { type: "string" }, createdAt: { type: "integer", description: "epoch-ms original event time" } } } } } }, responses: { "200": { description: "{ id, dim, merged? }", ...jsonObj }, "400": { description: "invalid tier / empty content" } } } },

      "/api/brain/recall": { post: { tags: ["Agent"], summary: "External recall surface (S39): hybrid-RRF query; ns '*' = admin cross-ns fan-out (BRAIN_ADMIN_XNS=1 + loopback double lock)", requestBody: { required: true, content: { "application/json": { schema: { type: "object", required: ["query"], properties: { query: { type: "string" }, k: { type: "integer", default: 5 }, ns: { type: "string" }, graphExpand: { type: "boolean" } } } } } }, responses: { "200": { description: "{ hits:[{id,tier,content,score,...}], crossNs? }", ...jsonObj }, "400": { description: "query required" }, "403": { description: "cross-ns without the double lock" } } } },
      "/api/brain/facts": { get: { tags: ["Agent"], summary: "Bi-temporal fact query (S40): live by default, ?at epoch-ms for a historical snapshot", parameters: [{ name: "subject", in: "query", required: true, schema: { type: "string" } }, { name: "ns", in: "query", required: false, schema: { type: "string" } }, { name: "at", in: "query", required: false, schema: { type: "integer" } }], responses: { "200": { description: "{ facts:[...] }", ...jsonObj }, "400": { description: "subject required" } } } },
      "/api/brain/session/{id}": { get: { tags: ["Agent"], summary: "Session ↔ episode reverse link (S42): memories written for this session (source=sessionId)", parameters: [{ name: "id", in: "path", required: true, schema: { type: "string" } }], responses: { "200": { description: "{ sessionId, memories:[...] }", ...jsonObj } } } },
