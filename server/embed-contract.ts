...
export function prefixPolicyFor(model: string): PrefixPolicy {
...
export function applyEmbedPrefix(text: string, role: EmbedRole, model: string): string {
  if (prefixPolicyFor(model) !== "nomic-v1") return text;
...
export function embedFingerprint(o: { provider: string; model: string; host: string }): string {
  const host = o.host.replace(/^https?:\/\//, "").replace(/\/+$/, "");
  return `${o.provider}:${o.model}@${host}/prefix=${prefixPolicyFor(o.model)}/norm=l2`;
}
...
export function contractEmbedder(
  ...
  return async (text, role = "query") => normalizeVector(await raw(applyEmbedPrefix(text, role, model), role));
}
