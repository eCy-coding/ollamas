...
- `--propose <stream>` — GROUNDED fleet proposal: inlines the stream's focus-file content so the model copies exact lines into a SEARCH/REPLACE block (deterministic → apply-ready), writes it to `~/.llm-mission-control/fleet/work/<stream>.<vendor>/PROPOSAL.md`, then triage/apply with `fleet-apply` like any worker. **Pool fail-over (vO60):** if gemini's day is spent, it routes the SAME grounded proposal to the best free-tier API vendor (`pickVendor` → groq/cerebras/zai via `scripts/agent-dispatch.mjs`) so the vendor production loop never stalls. Proven live: gemini-2.5-flash → safe-auto shipped (vO57); gemini 20/20 → pool→groq → real apply-ready safe-auto SR proposal (vO60).
