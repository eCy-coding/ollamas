...
| **Fleet-apply** | Conductor apply-readiness triage: extracts each gated proposal's diff, dry-runs `git apply --check`, reports apply-ready vs illustrative → `FLEET_APPLY.md`. `--apply <stream>.<slot>` applies one + gates (tsc+test) + keeps-on-green/reverts-on-red. Closes the produce→gate→APPLY loop; never blind-applies weak diffs | `orchestration/bin/fleet-apply.ts` + `bin/lib/fleet-apply.ts` | `/fleet-apply` |
