...
import { GoogleCalendarBrowser } from '../../src/components/GoogleCalendarBrowser';

const DISABLED_BODY = JSON.stringify({
  error: {
    code: 403,
    message:
      'Google Calendar API has not been used in project 393156926657 before or it is disabled. Enable it by visiting https://console.developers.google.com/apis/api/calendar-json.googleapis.com/overview?project=393156926657 then retry.',
  },
});
...
describe('GoogleCalendarBrowser', () => {
  it('renders connect prompt when logged out', () => {
    renderUI(<GoogleCalendarBrowser />);
  ...
  it('shows persistent enable-API error on 403-disabled and fetches exactly once (loop regression)', async () => {
    ...
    await screen.findByText(/Calendar API is disabled/i);
    const link = screen.getByRole('link', { name: /Enable the Google Calendar API/i });
    expect(link.getAttribute('href')).toContain('calendar-json.googleapis.com');
