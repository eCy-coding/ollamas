...
describe("router integration — 429 with Retry-After cools the spent key", () => {
  ...
  it("catalog provider 429 → the SPENT key leaves the live pool (singleAttempt surfaces the error)", async () => {
    ...
    // The 429 surfaces honestly (singleAttempt = no fallback masking). The cooldown TTL math
    // itself is covered deterministically by the quotaCooldownTtl unit tests above; asserting
    // an absolute pool count here would depend on the operator's real vault + persisted
    // cross-run cooldowns (the stub keyId is deterministic, so it stays cooled between runs).
