...
// vF13 — `color-contrast` re-enabled. vF14 — scanned across BOTH themes (the
// no-flash script derives data-theme from prefers-color-scheme, which Playwright
// sets via colorScheme), so light-theme status colors are gated too.
...
for (const scheme of SCHEMES) {
  test.describe(`${scheme} theme`, () => {
    ...
    for (const tab of TABS) {
      test(`a11y [${scheme}]: "${tab}" has no critical/serious WCAG AA violations`, async ({ page }) => {
        await page.goto('/');
        ...
        expect(blocking, `axe violations on ${tab} [${scheme}]: ${JSON.stringify(summary, null, 2)}`).toEqual([]);
      });
    }
  });
}
