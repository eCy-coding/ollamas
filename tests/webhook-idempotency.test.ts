...
describe("stripe webhook idempotency (H6)", () => {
  test("a handler FAILURE does not consume the event (Stripe can retry)", async () => {
    const t = await store.createTenant("whtenant", "pro");
    const r = await billing.handleWebhook(evt("evt_fail", "nonexistent_plan", t.id), "sig");
    expect(r.handled).toBe(false); // setTenantPlan threw on unknown plan, caught
    expect(await store.stripeEventSeen("evt_fail")).toBe(false); // NOT consumed → retryable (pre-fix: true)
  });
