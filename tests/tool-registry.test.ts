...
describe("ToolRegistry choke-point", () => {
  test("exposes all 35 built-in workspace tools", () => {
    expect(ToolRegistry.schemas().length).toBe(35); // +bench_model(v1.8) +mac_power(v1.9) +eval_prompt(v1.12) +rag_index/rag_search(v1.13) +count_tokens(graft) +upload_file/download_file(updown) +test_generate/code_audit/storefront_generate(revenue-ops Faz19) +contract_admin(contract vK2)
  });
