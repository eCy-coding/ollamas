...
describe("ToolRegistry choke-point", () => {
  test("exposes all 31 built-in workspace tools", () => {
    expect(ToolRegistry.schemas().length).toBe(31); // +bench_model(v1.8) +mac_power(v1.9) +eval_prompt(v1.12) +rag_index/rag_search(v1.13) +count_tokens(graft) +upload_file/download_file(updown)
  });
