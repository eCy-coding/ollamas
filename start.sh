...
die() {
  printf '\033[31m[up] HATA:\033[0m %s\n' "$*" >&2
  exit 1
}
st_ollama="-"
st_bridge="-"
...
if [ ! -f .env ]; then
  if [ "$DRY_RUN" = "1" ]; then printf '\033[35m[DRY]\033[0m would run: cp .env.example .env\n'; else
    cp .env.example .env
    warn ".env oluşturuldu — key için ./setup-keys.sh"
  fi
fi
keyn=0
for k in GEMINI OPENAI OPENROUTER OLLAMA_CLOUD; do
