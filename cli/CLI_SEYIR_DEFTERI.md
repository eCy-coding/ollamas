...
## Feature — 3-makine filosu (1 Mac + 2 Windows), minimum-manuel failover (2026-06-28)
**İstek**: 3 bilgisayar minimum manuel işlemle tek sistem gibi. Mac=kontrol düzlemi, 2 Windows=GPU worker; failover (öncelik-sıralı); kapsam CLI+bootstrap (server değişmez). Remote-GPU feature'ı (159ddb7) pool+failover'a genişletir.
**Phase/todo (tamamlandı):**
1. `cli/lib/remote.ts` +SAF: `Backend`/`parseBackendPool`/`selectBackend`(öncelik-sıralı failover çekirdeği)/`parseTailscalePeers`(auto-discovery çekirdeği)/`formatPool`. `cli/lib/fleet.ts` NEW SAF: `decideTransition`(stay/switch/wait/stop, thrash-guard minDwell, backoff=watch.ts nextBackoff).
2. `cli/commands/remote.ts` thin-IO: discover (`tailscale status --json` execFile→:11434 prob→pool auto), add/rm/ls, check --all, pick, up [--watch --exec --interval --min-dwell --required] supervisor (gateway child spawn OLLAMA_HOST=best, düşerse SIGTERM→SIGKILL→re-pick→respawn). Pool `~/.ollamas/backends.json`.
3. Bootstrap: `scripts/fleet-join.ps1` (Windows idempotent: Tailscale/Ollama/0.0.0.0-bind/firewall/pull), `scripts/fleet-up.sh` (Mac: ready→discover→up --watch). Runbook `cli/FLEET.md`.
4. gate + cli-verifier APPROVED.
**Kanıt**: tsc 0 · 39 fleet/remote test (912 full pass, 5 pre-existing infra-fail) · cli-verifier APPROVED (injection-safe execFile/spawn-array no-shell, failover doğru, child-lifecycle temiz) · **canlı discover Mac'i tailnet'te buldu** (emre-macbook-pro 100.108.67.76, 18 model, qwen3:8b ✓, elle-IP-yok) · choke-point korunur · zero-dep.
**Rakip/SWOD**: ollama-pool+health-failover yerleşik desen yok (kullanıcılar nginx/litellm-router elle kurar); ayırt edici = Tailscale-auto-discovery (sıfır-IP) + priority-failover supervisor + tek-komut/makine bootstrap. p2p_network.go DHT scaffolding KULLANILMADI (server'a bağlı değil).
- **N-032 (footgun)**: failover dürüst sınırı = gateway OLLAMA_HOST'u boot'ta okur + CURRENT_MODE cache (server.ts:168/188) → backend-geçişi RELAUNCH (~sn), zero-downtime DEĞİL. Canlı in-flight reroute server-içi pool ister (minimal-scope dışı). Home-fleet için doğru trade.
- **N-033 (güvenlik)**: harici süreç çağrıları (`tailscale`, gateway-child) execFile/spawn ARRAY-args, `shell:true` ASLA — DNSName/URL shell'e interpolasyon yok (injection savunması). `--exec` argv-split (tırnak-arg taşımaz, dokümante sınır).
- **N-034 (thrash-guard)**: failover supervisor minDwell (default 10s) olmadan flap eder; `decideTransition` lastSwitchMs+minDwell ile rapid-switch'i `wait`'e çevirir; all-down=`wait(backoff)` (stop DEĞİL → kalıcı retry).

### Canlı kalibrasyon + N-035 bug fix (2026-06-28)
**Canlı 2-makine kalibrasyon (Mac + desktop-ert7724):** tailscale mesh `emrecnyn@` (direct LAN, 60ms) · Mac ollama+qwen3:8b VRAM · supervisor `up --watch --exec` PROVEN (spawn→child OLLAMA_HOST enjeksiyonu→temiz SIGTERM). **Windows worker uzaktan attach (Windows-login YOK):** ollama `/api/pull` tailnet'ten açık → `curl http://<peer>:11434/api/pull -d '{"model":"qwen3:8b"}'` ile 5.2GB modeli Mac'ten çektirildi (RDP/SSH/USB yok) → `remote discover` → `pick` → **gerçek GPU inference `FLEET_OK` ~24-47 tok/s** (VRAM-resident). Teknik: ollama-pull-over-HTTP = worker'ı uzaktan provision et.
- **N-035 (bug+fix, kanıt)**: `discover` öncelik aritmetiği BOZUKTU — `cli/commands/remote.ts` inline `workerPriority++ * 10` (init **10**) → worker'lara **100,110,…** veriyordu = Self'in 99'undan KÖTÜ → `selectBackend` gerçek GPU worker'ı ASLA seçmiyor (hep Mac-fallback). Yorum niyeti 10,20,30. **Fix**: saf fn `assignDiscoveredPriorities(peers, selfHost)` → `cli/lib/remote.ts` (workerIndex **1**'den → 10,20,30; Self 99), inline mantık çıkarıldı. TDD 4 test (`tests/cli-fleet.test.ts`, RED→GREEN, fn-revert ile gate doğrulandı). **Kanıt**: tsc 0 · fleet 30/30 · cli-verifier APPROVE · **canlı: boş pool → tek `discover` → desktop-ert7724 otomatik pri 10** (manuel rm+add ARTIK YOK). Residual (follow-up, kapsam-dışı): merge `existingByUrl.set(d.url,d)` URL-bazlı → worker IP değişirse eski URL'li stale girdi kalır.
