...
| **v18** | minisign imza (authenticity) | `cli/lib/minisign-verify.ts` SAF zero-dep doğrulama (node:crypto blake2b512+Ed25519, **minisign-binary user'da GEREKMEZ**; file-sig + global/trusted-comment dual-guard) + `update` sig-gate (decideSigGate **fail-closed**: pinned+sig-fail/unsigned→ABORT, env-bypass YOK) + manifest minisig/keyId + pubkey.ts pinned(TOFU/rotasyon) + `cli/sign-release.sh` (minisign -Sm, key-yoksa abort) + release-binary.yml secret-gated step. 14 test; tsc 0; verifier APPROVED | ✅ DONE |
| **v19** | i18n/a11y completeness | ReactAgentTab a11y: nested-interactive→stretched-link (kart pasif + içte stretched select-button + delete z-10), color-contrast→chat-bubble `text-white` (light 2.76→6.46:1); i18n: ~40 string → 65 `react-agent.*` key (en/tr PARITY) + `useLingui` `_()`, 0 hardcoded. Playwright axe **8/8 (4 tab×2 tema, 0 serious)**; tsc 0; 873 test; verifier APPROVED | ✅ DONE |
| **v20** | enterprise/GA · audit export | `ollamas saas audit export [--tenant] [--format csv\|jsonl\|json] [--since/--until/--limit] [-o]` — gateway `/api/saas/audit` verisini SOC2/SIEM/S3 için CSV/JSONL/JSON dosya-export (RFC-4180 + 0600); `cli/lib/audit.ts` saf. ≤1000-satır server-cap (N-043) | ✅ DONE |
| **v21** | Siri arama yardımcısı (on-device, server-siz) | `ollamas siri <soru> [--say]` → `bin/siri-ask.mjs` beyni: Truth-Oracle deterministik "Doğru/Yanlış" YA DA deep `web_search` (`--deep/--top/--render` + jsdom readability motoru `web-extract.mjs`) + qwen3:8b Türkçe sentez (≤60 kelime +kaynak), cache/trace/multi-turn/extractive-fallback. `recipeSiri`/`askAction`/`runShellAction`/`speakAction` (`shortcuts.ts`) → yerel Run-Shell Siri Shortcut plist + kurulum kartı (`scripts/siri-shortcut-gen.ts`). Thin launcher (N-012-safe: subprocess, server import YOK). tsc 0; siri-ask 34 + web-search 16 test | ✅ DONE |
| **v20+** | Ufuk (önceden-hesap) | usage/timeseries-CLI · audit-cursor (cross-lane) · RBAC/SLA (server-endpoint-prereq) · webhooks-CLI · billing-portal | |
...
- **CANLI KALİBRASYON (2026-06-28)**: Mac control + desktop-ert7724 GPU worker canlı doğrulandı. Worker uzaktan attach (Windows-login YOK): ollama `/api/pull` tailnet'ten → 5.2GB model Mac'ten çektirildi → `discover`/`pick` → **gerçek GPU inference `FLEET_OK` ~24-47 tok/s**. **N-035 fix**: `discover` worker'lara 100/110 yerine doğru 10/20/30 atıyor (saf `assignDiscoveredPriorities`, TDD 4 test, fleet 30/30, cli-verifier APPROVE) → GPU worker artık otomatik tercih ediliyor. Kalan: 2. Windows tailnet-join + canlı worker-kill failover demo.

## v20 — enterprise/GA · `saas audit export` — DONE (kanıt) (2026-06-28)
- **Kritik-gap tespiti** (precompute: enterprise/GA RBAC/multi-tenant/audit-export/SLA → "capability-audit, en kritik non-redundant gap"): server `GET /api/saas/audit` (server.ts:1839 · `listAudit` store/index.ts:270, ≤1000 satır, alanlar `id,tenant_id,tool,tier,ok,ts`) veriyi sunuyor ama CLI `saas audit` yalnız TTY tablo basıyordu → **dosya export YOK (CSV/JSONL), date-range YOK** → SOC2/SIEM/S3 log-shipping bloklu. RBAC/SLA = server-endpoint yok → cross-lane (CLI-only inşa edilemez) → audit-export tek in-scope gerçek gap. v15-backup deseni (0600 file + format dönüşümü).
- **Adoption** (lisans-disiplinli, zero-dep): RFC-4180 CSV (yapısal escaping) + JSONL (SIEM line-delimited) fikir-only; in-repo `client.listAudit` (client.ts:450) + v15 `backupOutName`/0600-write deseni AYNEN reuse. Yeni dep yok.
- `cli/lib/audit.ts` (NEW saf): `toCsv`(RFC-4180 `csvField` quote/double), `toJsonl`(empty→no-stray-newline), `filterByDate`(inclusive ISO; bare-`until`→gün-sonu widen), `auditExportName`(`:`/`.`→`-` + tenant filename-safe, path-traversal-yok), `formatAudit`/`isAuditFormat` dispatch. `commands/saas.ts`: `audit export` subaction (`--format csv|jsonl|json` authoritative ≠ global `--json`, `--since/--until/--limit`, `-o`→0600 file / yoksa stdout) + HELP.
- **Server cap**: `listAudit` 1000-satır tavan, cursor YOK → export ≤1000 en-yeni event; gerçek bulk/cursor export = gelecek cross-lane server değişikliği (N-043).
- Testler: `cli-audit-export`(18 saf + mock-fetch round-trip: gerçek 0600 dosya yazımı `statSync` ile doğrulandı + JSONL-stdout+date-filter + unknown-format exit-2) → CLI lane **401 pass**. tsc 0; lint 0; **cli-verifier APPROVED**. VERSION değişmedi (v17-19'la tutarlı; bump = outward-facing operatör kararı). Choke-point korunur (registry import YOK); zero-RUNTIME-dep. **Gotcha**: N-043 audit-1000-cap-no-cursor; N-044 CSV-yapısal-escaping-var-ama-Excel-formula-content-injection-YOK (alanlar pratikte enum/hex/ISO → teorik); N-045 export-`--format`-authoritative-global-`--json`-yok-sayılır.

**Precompute (→v21)**: enterprise/GA devam — RBAC/SLA CLI-only inşa edilemez (server-endpoint yok); en kritik in-scope sıradaki ya (a) `usage/timeseries` CLI yüzeyi (server var `GET /api/saas/usage/timeseries`, CLI sıfır → forecast/capacity) ya da (b) cross-lane server audit-cursor kararı (N-043) → ilk todo: capability-audit (timeseries-gap vs cursor-prereq).
