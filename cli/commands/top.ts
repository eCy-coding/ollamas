...
import { resolveOutputCtx, c, formatTable, sparkline, compactNum, type OutputCtx } from "../lib/output";

const HELP = `ollamas top — live gateway metrics dashboard

  top [--watch] [--interval <s>] [--json]

  (snapshot by default; --watch repaints every <s> seconds, default 2)
  /metrics needs no auth; the usage panel needs a tenant key (OLLAMAS_API_KEY).
  seyir tail reads ~/.llm-mission-control/seyir-defteri.jsonl on THIS host.
...
// Pure: render the dashboard frame. ctx.json is handled by the caller (raw data).
export function renderDashboard(s: Snapshot, ctx: OutputCtx): string {
...
export async function runTop(argv: string[]): Promise<number> {
  ...
  if (!values.watch) {
    try {
      ...
      process.stdout.write((ctx.json ? JSON.stringify(snap, null, 2) : renderDashboard(snap, ctx)) + "\n");
      return 0;
    } catch (e: any) {
  ...
  const tick = async () => {
    try {
      ...
      process.stdout.write(CURSOR_HOME + CLEAR + renderDashboard(snap, ctx) + "\n");
    } catch (e: any) {
